"""
Testes para o scraper do Bloomberg Brasil.
...
@pytest.fixture(scope="module")
def bloomberg_scraper(scraper):
    """Fixture com BloombergScraper."""
    return BloombergScraper(scraper)


def test_bloomberg_get_latest_articles(bloomberg_scraper):
    """Testa coleta de URLs da homepage."""
    urls = bloomberg_scraper.get_latest_articles(limit=5)
    
    assert len(urls) > 0, "Deve retornar pelo menos 1 URL"
    assert len(urls) <= 5, "Não deve exceder o limite"
    
    for url in urls:
        assert "bloomberg.com.br" in url
        assert url.startswith("http")
        assert len(url) > 50, "URLs de artigos devem ser longas"


def test_bloomberg_mercados_category(bloomberg_scraper):
    """Testa coleta de artigos de Mercados."""
    urls = bloomberg_scraper.get_mercados_articles(limit=3)
    
    assert len(urls) > 0
    for url in urls:
        assert "/mercados/" in url or "bloomberg.com.br" in url


def test_bloomberg_extract_metadata(bloomberg_scraper):
    """Testa extração de metadados completos de um artigo."""
    # Coletar uma URL recente
    urls = bloomberg_scraper.get_latest_articles(limit=1)
    assert len(urls) > 0, "Deve ter pelo menos 1 URL"
    
    url = urls[0]
    
    # Acessar a página do artigo
    bloomberg_scraper.scraper.get_page(url, wait_time=3)
    
    # Extrair metadados
    article = extract_article_metadata(url, bloomberg_scraper.scraper.driver)
    ...
    assert "bloomberg" in article.source.lower()
...
def test_bloomberg_multiple_articles_metadata(bloomberg_scraper):
    """Testa extração de metadados de múltiplos artigos."""
    urls = bloomberg_scraper.get_latest_articles(limit=3)
    ...
    for url in urls:
        bloomberg_scraper.scraper.get_page(url, wait_time=2)
        article = extract_article_metadata(url, bloomberg_scraper.scraper.driver)
