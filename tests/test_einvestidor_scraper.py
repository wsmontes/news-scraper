"""
Testes para o scraper do E-Investidor (Estadão).
...
from news_scraper.sources.pt import EInvestidorScraper
...
@pytest.fixture(scope="module")
def einvestidor_scraper(scraper):
    """Fixture com EInvestidorScraper."""
    return EInvestidorScraper(scraper)


def test_einvestidor_get_latest_articles(einvestidor_scraper):
    """Testa coleta de URLs da homepage."""
    urls = einvestidor_scraper.get_latest_articles(limit=5)
    
    assert len(urls) > 0, "Deve retornar pelo menos 1 URL"
    assert len(urls) <= 5, "Não deve exceder o limite"
    
    for url in urls:
        assert "einvestidor.estadao.com.br" in url
        assert url.startswith("http")
        assert len(url) > 60, "URLs de artigos devem ser longas"


def test_einvestidor_investimentos_category(einvestidor_scraper):
    """Testa coleta de artigos de Investimentos."""
    urls = einvestidor_scraper.get_investimentos_articles(limit=3)
    
    assert len(urls) > 0
    for url in urls:
        assert "/investimentos/" in url


def test_einvestidor_extract_metadata(einvestidor_scraper):
    """Testa extração de metadados completos de um artigo."""
    # Coletar uma URL recente
    urls = einvestidor_scraper.get_latest_articles(limit=1)
    assert len(urls) > 0, "Deve ter pelo menos 1 URL"
    
    url = urls[0]
    
    # Acessar a página do artigo
    einvestidor_scraper.scraper.get_page(url, wait_time=3)
    
    # Extrair metadados
    article = extract_article_metadata(url, einvestidor_scraper.scraper.driver)
    ...
    assert "estadao" in article.source.lower() or "investidor" in article.source.lower()
...
def test_einvestidor_multiple_articles_metadata(einvestidor_scraper):
    """Testa extração de metadados de múltiplos artigos."""
    urls = einvestidor_scraper.get_latest_articles(limit=3)
    ...
    for url in urls:
        einvestidor_scraper.scraper.get_page(url, wait_time=2)
        article = extract_article_metadata(url, einvestidor_scraper.scraper.driver)
